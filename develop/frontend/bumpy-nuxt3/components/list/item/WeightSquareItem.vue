<template>
  <li class="square-item bp-pa-sm bp-ma-sm" @click="moveDetail">
    ...
    <div class="bp-mt-md bp-mb-lg">
      <p>
        <span>{{ info.weightStart }}{{ info.measure==='1' ? 'kg' : 'lb' }}</span> ~ <span>{{ info.weightEnd }}{{ info.measure==='1' ? 'kg' : 'lb' }}</span> / <span>{{ info.pollWeight }}kg</span>
      </p>
      <p>
...
const moveDetail = async () => {
  await weightStore.getSelectItemByStdDateSeq(weightStore.getFocusDate(), props.info.seq)
